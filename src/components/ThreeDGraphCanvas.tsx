...
// Subject colors - highly vibrant neon/pastel HUD color palette to pop on dark canvas
...
export const getLightningOffset = (y: number) => {
...
export default function ThreeDGraphCanvas({
  ...
  const nodes = useMemo(() => {
    return topicsList.map((topic, index) => {
      ...
      return {
        ...
        color: SUBJECT_COLORS[topic.subject] || "#94a3b8",
        baseRadius,
        isMilestone,
        age
      };
  ...
  useEffect(() => {
    ...
    if (activeTopic) {
      for (const dep of dependenciesList) {
        ...
        if (isPrereqPath) {
          ...
          if (fromNode && toNode) {
            ...
            const edgeColor = new THREE.Color(SUBJECT_COLORS[fromNode.topic.subject] || "#ffffff");
      ...
      const activeColor = SUBJECT_COLORS[activeTopic.subject] || "#ffffff";
      ...
      if (tooltipRef.current) {
        if (currentHovered) {
          ...
          if (proj) {
            ...
            const subjectColor = SUBJECT_COLORS[currentHovered.subject] || "#94a3b8";
            if (tooltipBarRef.current) tooltipBarRef.current.style.backgroundColor = subjectColor;
            if (tooltipDomainRef.current && tooltipDomainRef.current.dataset.id !== currentHovered.id) {
              ...
              tooltipDomainRef.current.style.color = subjectColor;
  ...
  return (
    ...
    >
      ...
      <div ref={labelBottomRef} className="absolute left-0 top-0 pointer-events-none text-[8.5px] font-mono text-pink-400/80 uppercase select-none leading-none tracking-wider" style={{ display: "none" }} />
      <div ref={labelMiddleRef} className="absolute left-0 top-0 pointer-events-none text-[8.5px] font-mono text-purple-400/80 uppercase select-none leading-none tracking-wider" style={{ display: "none" }} />
      <div ref={labelTopRef} className="absolute left-0 top-0 pointer-events-none text-[8.5px] font-mono text-cyan-400/80 uppercase select-none leading-none tracking-wider" style={{ display: "none" }} />

      {/* Smart node-label layer (populated imperatively from the label pool). */}
      <div ref={labelLayerRef} className="absolute inset-0 pointer-events-none z-30 select-none" />
      ...
      <div className="absolute top-3 right-3 z-40 flex gap-1.5">
        <button
          ...
          className={`pointer-events-auto flex items-center justify-center w-9 h-9 rounded-lg border backdrop-blur-md transition-colors cursor-pointer ${
            autoRotate
        ...
        <button
          ...
          className="pointer-events-auto flex items-center justify-center w-9 h-9 rounded-lg border bg-[#070b19]/70 border-white/10 text-slate-300 hover:text-white hover:border-white/20 backdrop-blur-md transition-colors cursor-pointer"
        >
